...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageBookingValue =
      recentBookings > 0 ? totalRevenue / recentBookings : 0;
...
function formatPrometheusMetrics(metrics: {
  ...
  const addMetric = (
    ...
    const labelStr = Object.entries(labels)
      .map(([key, val]) => `${key}="${val}"`)
      .join(',');
    lines.push(`kivvy_${name}{${labelStr}} ${value}`);
  };
  ...
  // Recent metrics
  addMetric('new_bookings', metrics.recent.newBookings, {
    period: metrics.period,
  });
  addMetric('new_users', metrics.recent.newUsers, { period: metrics.period });
  addMetric('revenue_total', metrics.recent.revenue, {
    period: metrics.period,
  });
  addMetric('average_booking_value', metrics.recent.averageBookingValue, {
    period: metrics.period,
  });
  ...
  // Category metrics
  metrics.categories.forEach((cat: { category: string; count: number }) => {
