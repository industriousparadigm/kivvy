...
export async function POST(request: NextRequest) {
  try {
    ...
    if (booking.payment.status !== 'COMPLETED') {
      return createErrorResponse('Payment must be completed to issue refund', 400)
    }
    ...
    const maxRefundAmount = booking.payment.amount - (booking.payment.refundAmount || 0)
    ...
    if (booking.payment.paymentMethodType === 'MBWAY') {
      ...
      return createSuccessResponse({
        message: 'MBWay refund processed. Customer will receive refund via bank transfer within 3-5 business days.',
        refundAmount,
