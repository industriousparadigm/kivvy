...
export function withMonitoring<T extends (...args: any[]) => Promise<NextResponse>>(
  ...
  return (async (request: NextRequest, ...args: any[]) => {
    ...
    try {
      ...
      // Track API response
      monitoring.trackApiResponse(method, endpointName, statusCode, duration, userId);
...
export function captureComponentError(error: Error, componentName: string, props?: Record<string, any>) {
  monitoring.captureError(error, {
