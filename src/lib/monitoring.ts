...
export class MonitoringService {
  ...
  // Error tracking
  captureError(error: Error, context?: Record<string, any>, user?: { id: string; email?: string }) {
  ...
  // Custom metrics
  captureMetric(name: string, value: number, unit: string = 'none', tags?: Record<string, string>) {
    // Log metric
  ...
  captureEvent(name: string, data?: Record<string, any>, user?: { id: string; email?: string }) {
  ...
  trackHealthCheck(service: string, status: 'healthy' | 'unhealthy', responseTime?: number) {
    ...
    this.captureMetric(`health.${service}`, status === 'healthy' ? 1 : 0, 'none', tags);
    
    if (responseTime !== undefined) {
      this.captureMetric(`health.${service}.response_time`, responseTime, 'millisecond', tags);
    }
  }
  ...
  // Add breadcrumb
  addBreadcrumb(message: string, category: string = 'custom', data?: Record<string, any>) {
    Sentry.addBreadcrumb({
      message,
      category,
      data,
...
export const captureError = (error: Error, context?: Record<string, any>, user?: { id: string; email?: string }) =>
  monitoring.captureError(error, context, user);

export const trackApiResponse = (method: string, endpoint: string, statusCode: number, duration: number, userId?: string) =>
  monitoring.trackApiResponse(method, endpoint, statusCode, duration, userId);

export const trackAuthEvent = (event: 'login' | 'logout' | 'register' | 'password_reset' | 'failed_login', userId?: string, metadata?: Record<string, any>) =>
  monitoring.trackAuthEvent(event, userId, metadata);

export const trackPaymentEvent = (event: 'initiated' | 'completed' | 'failed' | 'refunded', amount: number, currency: string, provider: string, userId?: string, bookingId?: string) =>
  monitoring.trackPaymentEvent(event, amount, currency, provider, userId, bookingId);

export const trackBookingEvent = (event: 'created' | 'confirmed' | 'cancelled' | 'completed', bookingId: string, activityId: string, userId: string, amount?: number) =>
